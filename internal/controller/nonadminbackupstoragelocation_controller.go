...
// the NonAdminBackupStorageLocation object against the actual cluster state, and then
...
func (r *NonAdminBackupStorageLocationReconciler) validateNaBSLSpec(ctx context.Context, logger logr.Logger, nabsl *nacv1alpha1.NonAdminBackupStorageLocation) (bool, error) {
	...
	if nabsl.Spec.Credential == nil || nabsl.Spec.Credential.Name == "" {
		err := fmt.Errorf("credentials secret name is required")
		logger.Error(err, "Validation failed")

		// Update status condition
		meta.SetStatusCondition(&nabsl.Status.Conditions, metav1.Condition{
	...
	if err := r.Get(ctx, types.NamespacedName{
		...
		if apierrors.IsNotFound(err) {
			logger.Error(err, "Credentials secret not found", "secretName", nabsl.Spec.Credential.Name)
			...
			meta.SetStatusCondition(&nabsl.Status.Conditions, metav1.Condition{
				...
				Message: fmt.Sprintf("Credentials secret %s not found in namespace %s", nabsl.Spec.Credential.Name, nabsl.Namespace),
			})

			if errStatus := r.Status().Update(ctx, nabsl); errStatus != nil {
				logger.Error(errStatus, failedUpdateStatusError)
	...
	// TODO: Add validation for the secret data and other BSL spec fields
...
func (r *NonAdminBackupStorageLocationReconciler) createVeleroBSL(ctx context.Context, logger logr.Logger, nabsl *nacv1alpha1.NonAdminBackupStorageLocation) (bool, error) {
	...
	op, err := controllerutil.CreateOrUpdate(ctx, r.Client, veleroBsl, func() error {
		veleroBsl.Spec.AccessMode = nabsl.Spec.AccessMode
		veleroBsl.Spec.BackupSyncPeriod = nabsl.Spec.BackupSyncPeriod
		veleroBsl.Spec.Config = nabsl.Spec.Config
		veleroBsl.Spec.Credential = &corev1.SecretKeySelector{
			LocalObjectReference: corev1.LocalObjectReference{
				Name: veleroBslSecret.Name,
			},
		}
		veleroBsl.Spec.Credential.Key = nabsl.Spec.Credential.Key
		veleroBsl.Spec.Default = nabsl.Spec.Default
		veleroBsl.Spec.ObjectStorage = nabsl.Spec.ObjectStorage
		veleroBsl.Spec.Provider = nabsl.Spec.Provider
		veleroBsl.Spec.ValidationFrequency = nabsl.Spec.ValidationFrequency
